...
for epoch in range(opt.niter):
    ...
    # do checkpointing
    if epoch % 100 == 99:
        vutils.save_image(real_cpu,'%s/real_samples_epoch_%05d.png' % (opt.outf, epoch+1),normalize=True)
        fake = netG(fixed_noise)
        vutils.save_image(fake.detach(),'%s/fake_samples_epoch_%05d.png' % (opt.outf, epoch+1),normalize=True)
