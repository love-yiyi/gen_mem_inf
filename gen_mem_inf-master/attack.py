...
for step in range(opt.niter):
    ...
    if step % 100 == 99:
        vutils.save_image(real_cpu,
                '%s/gen_real_samples_step_%05d.png' % (opt.outf, step+1),
                normalize=True)
        fake = netG(fixed_noise)
        vutils.save_image(fake.detach(),
                '%s/gen_fake_samples_step_%05d.png' % (opt.outf, step+1),
                normalize=True)

# do checkpointing
torch.save(netWBG.state_dict(), '%s/netWBG_step_%d.pth' % (opt.outf, step+1))
torch.save(netWBD.state_dict(), '%s/netWBD_step_%d.pth' % (opt.outf, step+1))
